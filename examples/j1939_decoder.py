...
class j1939_decoder(object):
    ...
    def decode(self, arbitration_id, can_data, matrix):
        if canmatrix.CanId(arbitration_id).pgn == 0xEC00 or canmatrix.CanId(arbitration_id).pgn == 0xECFF:
            # BAM detected
        ...
        if canmatrix.CanId(arbitration_id).pgn == 0xEB00 or canmatrix.CanId(arbitration_id).pgn == 0xEBFF:
